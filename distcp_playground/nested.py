...
class RenameSaver(DefaultSavePlanner):
    """
    Example SavePlanner that uses flatten_state_dict to handle complex
     state_dict objects
    """
    ...
    def create_global_plan(
        ...
        merged_mappings = reduce(
            lambda x, y: x | y,
            (p.planner_data for p in global_plan)
        )
        metadata = dataclasses.replace(
            metadata,
            planner_data=merged_mappings
        )
...
class RenameLoader(DefaultLoadPlanner):
    """
    Use flatten_state_dict to handle complex state_dict objects.
    """
    def init(
        self,
        state_dict: STATE_DICT_TYPE,
